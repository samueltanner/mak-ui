...
import { makUiDefaultColors, makUiDefaultStateShades, makUiDefaultThemeShades, makUiPalettes, makUiShadesSet, makUiThemesSet, makUiVariants, mediaQueries, twModifierSet, twToCssKeyMap, } from "../constants/ui-constants";
...
export const parseClassNameToStyleObject = ({ className = "", makClassName = undefined, activeTheme, }) => {
...
export const extractMakVars = ({ className, activeTheme, }) => {
    var _a, _b, _c, _d;
    ...
    variant =
        (mcn === null || mcn === void 0 ? void 0 : mcn.split("-")[1]) || "primary";
    if (variant.includes("|")) {
...
const findSubstring = ({ string, substrings, }) => {
    if (!string)
        return undefined;
...
const constructCSSClassNameObject = ({ makClassName, activeTheme, rootClassObject, returnAllVars, }) => {
    let { className, modifiersArray, mediaQueriesArray, relationalModifiersArray, } = separateTwModifiers(makClassName);
...
const parseGradientClassNames = ({ makClassName, activeTheme, }) => {
    var _a, _b;
    ...
    for (let instruction of gradientModifierObject.instructions) {
        ...
        const gradientDirection = gradientType === "linear"
            ? `${linearGradientDirections === null || linearGradientDirections === void 0 ? void 0 : linearGradientDirections[direction]}, `
            : gradientType === "radial, "
                ? "circle, "
                : "";
    ...
    const hasViaValues = gradientModifierObject.viaColors.length > 0 ||
    ...
    for (let terminus of ["from", "via", "to"]) {
        for (let twPosition of gradientModifierObject[`${terminus}Positions`]) {
            if (!gradientModifierObject[`${terminus}Positions`].length)
                continue;
    ...
    for (let terminus of ["from", "via", "to"]) {
        for (let twColor of gradientModifierObject[`${terminus}Colors`]) {
            ...
            if (terminus === "from") {
                ...
                if (hasViaValues) {
                    rootClassObject = Object.assign(Object.assign({}, rootClassObject), { "--gradient-via": `var(--gradient-via-color) var(--gradient-via-position, 50%)` });
                }
            }
            else {
                rootClassObject = {
...
export const parseMakClassNames = ({ makClassName, activeTheme, }) => {
